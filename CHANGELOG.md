...
- **Network settings page.** A full network editor (`Network` in the menu, once past first-run setup) puts everything the wizard decides once — in one place, changeable any time, in tabbed sections: **Mode** (router ⇄ Wi-Fi extender), **Internet** (WAN source: an explicit Ethernet-port or cellular-modem choice — extender shows an upstream Wi-Fi picker with scan), **Wi-Fi** (SSID/password/band/channel), **Ports** (switch spare wired Ethernet ports into the LAN — bridged with Wi-Fi into one network), and **Local network** (LAN subnet + DHCP). Backed by `GET`/`PUT /api/network` (+ `GET /api/network/scan`); every change runs through the transactional apply path (snapshot → apply → health-check → **auto-rollback if the AP doesn't come back up**). The first-run wizard's connection step also gained wired-LAN-port assignment when more than one cabled port is present (`network.lan_ports` config field). _Wired-LAN bridging still needs on-device validation._
- **5 GHz Wi-Fi on Pi 4/5.** The AP band was previously hardwired to 2.4 GHz (hostapd `hw_mode=g`, the `band` field was ignored). It now honours the band: 5 GHz uses `hw_mode=a` with 802.11ac / 80 MHz VHT on the non-DFS UNII-1 channels (36/40/44/48). The band picker (2.4 / 5 GHz) appears in both the settings page and the wizard, but only on boards whose radio supports it (`five_ghz_capable` = Pi 4/5); 2.4-only boards (Zero 2W, Pi 3B) don't show it. The channel list follows the band. Note the Pi's single built-in radio is **one band at a time** — simultaneous dual-band would need a second Wi-Fi adapter. _5 GHz AP needs on-device validation on Pi 4/5 (brcmfmac)._
  - This **replaces** the earlier narrow "Ethernet ports" page. That page's WAN assignment was implicit and could silently switch a working cellular WAN off to Ethernet; the new WAN source is an explicit, mutually-exclusive choice, so editing ports (or anything else) can never drop your modem by accident.

### Fixed

- **Telegram bot token can be saved where `api.telegram.org` is blocked.** Saving the token restarted the bot, which validates it with `getMe` over the HTTP Bot API — in blocked regions that times out (`context deadline exceeded`) and the save hard-failed with a red error, even though the token was already stored. The token now saves regardless; a failed start is a **soft yellow warning** that points you to enable the local Telegram proxy + set `app_id`/`app_hash` so the bot runs over MTProto. (`PUT /notify/telegram/token` and `/app` return 200 + `warning` instead of 400.)
- **Wired LAN ports appear even without a cable plugged in.** The `Network → Ports` tab only listed ports with a live cable, so on a modem WAN with a single free onboard port there was nothing to assign. A LAN port is assigned first and cabled later, so it now lists every port that isn't the WAN (with a cable-state hint).
...
- **Image build no longer fails at compression under memory pressure.** The `[7/7]` step could be OOM-killed silently (0-byte output, bare "Build failed") because the page cache filled by the mount/chroot/`cp` steps plus `xz -T0`'s buffers tipped a memory-capped VM (WSL2) over. The build now drops the page cache and caps `xz` memory (`--memlimit-compress=40%`) before compressing, writes to a `.partial` promoted only on success, and prints the real exit code on failure.
- **Image artifact renamed** from `KnotOS-zero2w-*` to `KnotOS-pi-arm64-*` — the image is generic arm64 Pi OS Lite and boots on any arm64 Pi (Zero 2W, 3, 4, 5); the old label misled Pi 4B/5 users. Cosmetic (nothing parses the name).
...
- **Motion pass + tab polish.** Switching tabs and navigating between pages now fades smoothly, and buttons have a subtle press response. The in-page tab strip no longer shows a stray scrollbar or nudges the page a pixel in either direction (hidden scrollbar, no negative margins). Dropped the "experimental" banner from the Modem page.

## [2026.07.5] — 2026-07-25

### Changed

- **In-page tabs — pages are no longer one long stack of cards.** New reusable `Tabs` sub-navigation (scrolls sideways on narrow screens so it never overflows). The **Modem** page is the first converted: **Status / Data / Network / Settings** tabs instead of eight stacked cards, so each screen is short and mobile-friendly. Rolling out to the other dense pages (Routing, System, Protection, VPN) next.
...
- **Cellular data usage + signal history.** The Modem page now shows how much data the modem has used this billing cycle (with an RX/TX breakdown), an optional monthly cap with a progress bar that warns when reached, and a signal-strength sparkline. Usage is read from the interface byte counters, accumulated across the modem's counter resets, and persisted so a reboot doesn't lose the month's total; the counter resets on a configurable billing day (1–28) to match your plan. New `wan.modem.data_limit_mb` / `wan.modem.cycle_reset_day` config, surfaced via `GET/PUT /modem`. The new `modemmetrics` package holds the (unit-tested) accounting; the modem watchdog samples counters + signal each tick.
